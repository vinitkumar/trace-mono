...
def pixel_pattern_for(ch: str) -> tuple[list[str], bool] | None:
    if ch in PIXEL_PATTERNS:
        return PIXEL_PATTERNS[ch], False
    if "a" <= ch <= "z":
        return PIXEL_PATTERNS[ch.upper()], True
    return None


def draw_pixel_pattern(pen: TTGlyphPen, cut: Cut, pattern: list[str], lower: bool) -> None:
    rows = len(pattern)
    cols = max(len(row) for row in pattern)
    height = 520 if lower else 700
    y0 = 0
    cell = min((cut.width - 2 * cut.side) / cols, height / rows)
    block = cell * 0.78
    total_width = cols * cell
    x0 = (cut.width - total_width) / 2
    top = y0 + height
    ...
    for row_index, row in enumerate(pattern):
        for col_index, value in enumerate(row):
            if value != "1":
                continue
            x = x0 + col_index * cell + (cell - block) / 2
            y = top - (row_index + 1) * cell + (cell - block) / 2
            rect(pen, round(x), round(y), round(block), round(block))
...
def draw_ascii_glyph(ch: str, cut: Cut):
    ...
    if ch in (" ", "\u00a0"):
        return empty_glyph()
    pixel_pattern = pixel_pattern_for(ch)
    if pixel_pattern is not None:
        pattern, lower = pixel_pattern
        draw_pixel_pattern(pen, cut, pattern, lower)
        return pen.glyph()
    if ch.isdigit():
